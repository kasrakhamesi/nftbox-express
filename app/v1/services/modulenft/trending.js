  ...
  static #getTopCollections = async (numberOfTrending = 500) => {
    try {
      ...
      const collectionsData = collectionsRequests
        .filter((item) => item.status === 200 && item.data.error === null)
        .map((item) => {
          return item.data.rankings
            .filter(
              (item) => item.logo_url !== null && item.statistics.floor !== null
            )
            .map((item) => {
              return {
      ...
      const finalBatchCollectionInfo = resBatchCollectionInfo?.content?.data
            ...
            try {
              if (
                ...
                finalCollectionData.push({
                  collection_slug: serializeCollection[j]?.collection_slug,
                  collection_name: serializeCollection[j]?.collection_name,
                  verified: serializeCollection[j]?.verified,
                  logo_url: serializeCollection[j]?.logo_url,
                  floor_price: serializeCollection[j]?.floor,
                  owners_count: serializeCollection[j]?.owners_count,
                  total_supply: serializeCollection[j]?.total_supply,
                  one_day_volume: serializeCollection[j]?.one_day_volume,
                  seven_day_volume: serializeCollection[j]?.seven_day_volume,
                  volume_traded: String(
                    parseInt(serializeCollection[j]?.volume_traded)
                  ),
                  contract_address:
                    finalBatchCollectionInfo[i]?.contract_address,
                  banner_image_url:
                    finalBatchCollectionInfo[i]?.banner_image_url,
                  description: finalBatchCollectionInfo[i]?.description,
                  image_url: finalBatchCollectionInfo[i]?.image_url,
                  twitter_url: finalBatchCollectionInfo[i]?.twitter_url,
                  discord_url: finalBatchCollectionInfo[i]?.discord_url,
                  website_url: finalBatchCollectionInfo[i]?.website_url,
                  telegram_url: finalBatchCollectionInfo[i]?.telegram_url,
                  instagram_url: finalBatchCollectionInfo[i]?.instagram_url,
                  percent_owner:
                    ...
                    serializeCollection[j]?.owners_count
                      ...
                      : String(
                          parseInt(
                            (parseInt(serializeCollection[j]?.owners_count) *
                              100) /
                              parseInt(serializeCollection[j]?.total_supply)
                          )
                        ),
                  is_trending: true,
                  collection_type:
