...
module.exports = {
    ...
    findOne : async(req,res) => { 
        ...
        return id ? response(await api.Get({where : condition , res : res , req : req }), res) : 
        api.Get({where : condition , res : res , req : req  , checkJwt : false, checkRole : false}).then(result => {
           return _.isEmpty(result.content.rows[0]) ? res.status(404).send({message : "Can't find this data."}) : res.status(200).send(result.content.rows[0])
        }).catch(e => {
