...
adminsPermissions.check = async (roleId, permissionName) => {
    try {
        const getPermissionsFromRoleId =
            await Model.models.admins_role_perm.findAll({
                where: {
                    roleId: roleId
                }
            })
        ...
        for (let i = 0; i < targetPermissions.length; i++) {
            for (let m = 0; m < permissionName.length; m++) {
                if (
                    permissionName[m] == targetPermissions[i].role ||
                    targetPermissions[i].role == 'GOD'
                )
                    return true
            }
        }
        ...
        return {
            status: 403,
            content: { message: "You Don't have permisssion for this endpoint" }
        }
    } catch {
        return {
            status: 403,
