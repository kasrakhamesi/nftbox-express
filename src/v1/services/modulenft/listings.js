...
module.exports.getListingsChangePercent = async () => {
  try {
    ...
    for (let k = 0; k < collections.length; k++) {
      try {
        await new this.GetListings(collections[k].contract_address).save()
      } catch (e) {
        console.log(e)
        continue
      }
...
module.exports.GetListings = class {
  ...
  save = async () => {
    try {
      const resFindCollection = await sequelize.models.collections.findOne({
        where: {
      ...
      const resNewListings = resAxiosNewListings.data.listings.map((item) => {
        return {
          ...
          timestamp: getTimestampFromIsoTime(item.eventTimestamp).toString()
        }
      })
      ...
      const _4hourListings = allListings
        .filter(
          (item) => parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 60 * 4
        )
        .map((item) => {
          return item
        })
      ...
      allListings.map((item) => {
        if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 1)
          ...
          timeframe._5min++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 10)
          timeframe._10min++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 15)
          timeframe._15min++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 30)
          timeframe._30min++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 60 * 1)
          timeframe._1hour++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 60 * 2)
          timeframe._2hour++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 60 * 6)
          timeframe._6hour++
        else if (parseInt(item.timestamp) <= Date.now() + 1000 * 60 * 60 * 12)
          timeframe._12hour++
        else if (
